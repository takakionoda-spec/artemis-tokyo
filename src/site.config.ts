...
export const siteConfig = {
  ...
  pipeline: {
    ...
    voice: {
      ...
      /** Composition rules. */
      compositionRules:
        '- Body: 4–7 short paragraphs in each language. Optionally include ONE "## subheading" line (e.g. "## What it means in Tokyo") and ONE "> pull-quote" line (a SHORT, direct sentence drawn from the original dispatch — quoting the source verbatim is encouraged, attributed generically as "the original report" / "原文より").\n- Do NOT use the article title as the first body paragraph — that is redundant. Open instead by setting a scene, framing a tension, or naming a concrete detail.\n- Bring ONE sensory image or concrete detail per piece — a specific texture, a measured distance, a named material, a domestic gesture, a price.\n- Do NOT fabricate statistics, quotes, names of real people, dates, or place names. If a fact is not in the source, omit it entirely. It is better to be quiet than wrong.\n- No URLs, no footnotes, no hashtags, no emojis.',
      ...
      closingBlock: {
        ...
        title: { en: "ARTEMIS TOKYO — A View from Tokyo", ja: "ARTEMIS TOKYO 視点" },
        /** Sub-heading rendered just below the title. */
        subheading: {
          en: "How this story reads from Tokyo.",
          ja: "東京から、この記事をどう読むか。"
        },
        ...
        rules:
          "Every article ends with a SEPARATE, signed editorial commentary block titled \"ARTEMIS TOKYO 視点\" (rendered apart from the main body by the front-end).\n\nThis block is NOT a summary. It is the magazine's own first-person reading of the dispatch *from Tokyo, in 2026*. It must:\n  - Address the reader directly as ARTEMIS TOKYO (e.g. \"ARTEMIS TOKYO の見立てでは…\" / \"From where we sit in Tokyo, this looks like…\").\n  - Compare or contrast the story against Japan's CURRENT realities: Japan's space programme (JAXA, H3, MMX, the Japanese Artemis astronaut selection), domestic industry (ispace, Astroscale, Synspective, PD AeroSpace, Interstellar), the urban texture of Tokyo (housing density, ageing population, design culture, hospitality, ramen-and-conbini infrastructure), or Japanese cultural sensibility (mono no aware, kintsugi, ma, wabi-sabi).\n  - Name a CONCRETE Japanese reference where natural — a neighbourhood, a company, a policy, an industry, a habit, a season. Do not force it.\n  - Be honest: if the news from abroad exposes a Japanese gap or advantage, say so plainly. No empty patriotism, no defeatism.\n  - Length: 3–5 paragraphs per language. Slightly more personal in voice than the body; the body reports, this block opines.\n\nBoth languages must contain a Japan-grounded comparison. The Japanese version is the primary one (this is Tokyo's own magazine); the English version is its parallel for foreign readers — write each natively."
      }
    }
  },
